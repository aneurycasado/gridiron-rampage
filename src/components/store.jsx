...
export const useStore = create((set, get) => ({
  ...
  plays: {
    selectedOffensivePlay: null,  // Currently selected offensive play
    selectedDefensivePlay: null,  // Currently selected defensive play
    playPhase: 'selection',       // 'selection', 'setup', 'active', 'complete'
    suggestedPlays: [],           // Coach's suggested plays
    formationPositions: {},       // Current formation player positions
    showPlaybook: false,          // Whether to show the play selection menu
    showRoutes: true,             // Whether to show route visualizations
  },
  actions: {
    // Play selection system actions
    selectPlay: (play, isOffensive = true) => {
      if (isOffensive) {
        set(state => ({ 
          plays: { 
            ...state.plays, 
        ...
        console.log(`Selected offensive play: ${play.name}`);
      } else {
        set(state => ({ 
          plays: { 
            ...state.plays, 
        ...
        console.log(`Selected defensive play: ${play.name}`);
      }
    },
    
    togglePlaybook: (show = null) => {
      set(state => ({ 
        plays: { 
          ...state.plays, 
          showPlaybook: show !== null ? show : !state.plays.showPlaybook 
        } 
      }));
    },
    
    setPlayPhase: (phase) => {
      set(state => ({ plays: { ...state.plays, playPhase: phase } }));
      console.log(`Set play phase to: ${phase}`);
    },
    
    toggleRouteDisplay: (show = null) => {
      set(state => ({ 
        plays: { 
          ...state.plays, 
          showRoutes: show !== null ? show : !state.plays.showRoutes 
        } 
      }));
    },
    
    setFormationPositions: (positions) => {
      set(state => ({ plays: { ...state.plays, formationPositions: positions } }));
    },
    ...
    startPlay: () => {
      ...
      const { downs, plays } = get();
      
      // Play hike sound at start of play
      try {
        playSound("hike.mp3", 0.8);
      } catch (e) {
        console.warn("Could not play hike sound", e);
      }
      
      // Set play phase to active
      const playPhase = 'active';
      ...
      set({ 
        ...
        plays: {
          ...plays,
          playPhase,
          showPlaybook: false // Close playbook when play starts
        }
      });
    ...
    endPlay: () => {
      ...
      const { downs, footballs, players, plays } = get();
      
      // Play whistle sound
      try {
        playSound("whistle.mp3", 0.6);
      } catch (e) {
        console.warn("Could not play whistle sound", e);
      }
    ...
    resetPlay: () => {
      ...
      const { downs, players, tacklePosition, plays } = get();
      ...
      set({ 
        ...
        plays: {
          ...plays,
          playPhase: 'selection',
          showPlaybook: true // Show playbook when play is reset
        }
      });
