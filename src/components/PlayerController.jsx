      ...
      // AI BEHAVIOR (for defensive player)
        ...
        if (!aiState.current.rushDelay) {
          ...
          if (targetPlayer && targetPlayer.position) {
            ...
            // Check for collision with offensive player who has the ball
            if (length < 3.0 && !hasCollidedWithOffense) {
              // Only tackle if this player has the ball
              const { activePossession } = useStore.getState();
              
              if (activePossession === "player-offense") {
                console.log("TACKLE: Ball carrier hit by defensive player!");
                ...
                // End the play
                actions.endPlay();
                console.log("Tackle! Play ended - ball carrier was tackled.");
                ...
                // This is not the ball carrier - just block
                console.log("Contact with non-ball carrier - blocking but play continues");
                
                // Apply blocking physics - slow down the defensive player
    ...
    >
      ...
      <group ref={visualRef} rotation={[0, rotationRef.current, 0]}>
        <KennyPlayer 
          ...
          // Use role-based model selection for main players too
          modelIndex={getRoleBasedModelIndex(id === "player-offense" ? "QB" : "LB", teamType)}
          direction={targetRotationRef.current}
          role={id === "player-offense" ? "QB" : "LB"}
        />
